...
## Deploy no Coolify — um único resource

O projeto inclui um `Dockerfile` de produção. Use **um único resource do tipo Dockerfile**: o Express serve a API e também o `dist` do React na mesma porta.

Variáveis mínimas:

```env
DATABASE_URL=postgresql://USUARIO:SENHA@HOST:PORTA/BANCO
DB_SCHEMA=schema_exclusivo_da_dupla
JWT_SECRET=uma_chave_grande_e_aleatoria
PORT=3333
```

Na inicialização do container, `npm --prefix backend run db:deploy` recria **somente** `DB_SCHEMA`, cria as tabelas, aplica o seed exigido pela avaliação e valida automaticamente o cenário. O script se recusa a recriar `public`. Depois disso a API sobe e serve o frontend.

Consulte `COOLIFY.md` para o passo a passo e as credenciais da demonstração.
