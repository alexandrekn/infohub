...
export const tarefasController = {
  ...
  async listarEntregaveis(req: Request, res: Response) {
    ...
    const podeAcessar =
      ...
      (usuario.perfil === "mentor" && (await equipesService.ehMentorDaEquipe(tarefa.id_equipe, usuario.id_usuario))) ||
      (usuario.perfil === "aluno" && (await equipesService.ehIntegranteDaEquipe(tarefa.id_equipe, usuario.id_usuario)));

    if (!podeAcessar) throw ApiError.forbidden();
